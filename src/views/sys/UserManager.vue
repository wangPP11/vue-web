...
<script lang="ts">
  ...
  export default class MenuMana extends Vue {
    ...
    setMenuParentName():void{
      ...
      let id = this.menu.getParentId();
      this.allMenus.forEach(ele => {
        if (ele.getId().toString() == id) {
          parentName = ele.getParentName();
        }else if (ele.getChilds() != null) {
          ele.getChilds().forEach(child=>{
            if (child.getId().toString() == id) {
              parentName = child.getParentName();
            }
          })
        }
      })
      this.menu.setParentName(parentName);
    }
    ...
    setParentId(data: Menu,node: any, eml:any): void{
      this.menu.setParentId(data.getId().toString());
      this.menu.setParentName(data.getName());
    }
  }
</script>
